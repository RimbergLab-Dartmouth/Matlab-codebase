function[err]=q_circle_error_calc_with_freq_flucs_and_angle(variable_params,data_real,data_imag,freq)
     freq_res_guess=variable_params(1);
     gamma_int_guess=variable_params(2);
     gamma_ext_guess=variable_params(3);
     sigma_guess = variable_params(4);
     angle_guess = variable_params(5);
...
%          disp('res freq too high')
     ...
     if sigma_guess>freq(end)-freq(1)
         err=1e9;
%          disp('gamma ext too high')
         return
     end
...
%      data_real=data_real';
%      data_imag=data_imag';
     [theory_real, theory_imag]=q_circle_with_freq_flucs_and_angle(freq,gamma_int_guess,gamma_ext_guess,freq_res_guess,sigma_guess,angle_guess);
%      size(theory_real)
%      size(data_real)
%      size(theory_imag)
%      size(data_imag)
