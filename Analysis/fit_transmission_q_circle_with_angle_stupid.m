function[fit_struct,flag,output]=...
    fit_transmission_q_circle_with_angle_stupid(data_log_mag,data_phase,freq_points,gamma_int_guess,gamma_ext_guess)
%%% 'stupid' refers to not accounting for the subtleties in the khalil
%%% paper. i.e., doing DCM instead of phi rm
%%% transmission_q_circle_error_calc_with_angle is the code we should use
...
%     end
    ...
    err= @(p) transmission_q_circle_error_calc_with_angle_stupid(p,data_real,data_imag,freq_points);
...
%     disp(['final error is ' num2str(goodness_fit)]);
    [theory_real,theory_imag] = transmission_q_circle_with_angle_stupid(freq_points,gamma_int_fit,gamma_ext_fit,freq_res_fit,angle_fit);%     plot(theory_real,theory_imag,'r')
%     hold on
    [data_lin_mag,data_phase_radians]=extract_lin_mag_and_phase_radians(freq_points,data_log_mag,data_phase);
    [data_real,data_imag]=extract_real_imag(freq_points,data_lin_mag,data_phase_radians);
