function [best_fit,theory_values, success, area_gaussian_left, area_gaussian_right, fit_error, gaussian_1_theory, gaussian_2_theory]=fit_2_gaussians_same_sigma_with_constraints(x_data,y_data, sigma_guess, mean_constraint)
    
%%%% mean constraint is a parameter that forces the two Gaussian peaks to
%%%% be apart by that amount.
...
%     sigma_guess = abs(x_data(fwhm_left_1) - x_data(fwhm_right));
    ...
    err= @(p) fit_2_gaussians_same_sigma_with_constraints_error_calc(p, x_data, y_data, mean_constraint);%mean((gaussian_theory(p,x_data)-y_data).^2);
...
%         end
        ...
        elseif wrapTo180(best_fit(2)) > wrapTo180(best_fit(5))
            area_gaussian_right = sum(best_fit(1).*exp(-(x_data - best_fit(2)).^2/2./best_fit(3).^2));
            area_gaussian_left = sum(best_fit(4).*exp(-(x_data - best_fit(5)).^2/2./best_fit(3).^2));
            gaussian_2_theory = best_fit(1) .*exp(-(x_data - best_fit(2)).^2/2./best_fit(3).^2);
            gaussian_1_theory = best_fit(4).*exp(-(x_data - best_fit(5)).^2/2./best_fit(3).^2);
        else 
