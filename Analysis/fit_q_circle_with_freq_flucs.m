function[fit_struct,flag,output]=...
    fit_q_circle_with_freq_flucs(data_log_mag,data_phase,freq_points,gamma_int_guess,gamma_ext_guess,sigma_guess)%,angle_guess)
...
%     end
    ...
    err= @(p) q_circle_error_calc_with_freq_flucs(p,data_real,data_imag,freq_points);
...
%     disp(['final error is ' num2str(goodness_fit)]);
    [theory_real,theory_imag] = q_circle_with_freq_flucs(freq_points,gamma_int_fit,gamma_ext_fit,freq_res_fit,sigma_fit);%     plot(theory_real,theory_imag,'r')
%     hold on
    [data_lin_mag,data_phase_radians]=extract_lin_mag_and_phase_radians(freq_points,data_log_mag,data_phase);
    [data_real,data_imag]=extract_real_imag(freq_points,data_lin_mag,data_phase_radians);
