...
%%%% freq, amp, phase
if ~exist('bias_point_struct', 'var')
    disp('load mat file with bias point struct')
    [file_name, file_path] = uigetfile;
    load([file_path file_name] ,'bias_point_struct')  
end

if ~exist('gain_prof', 'var')
    disp('load mat file with gain profile')
    [file_name, file_path] = uigetfile;
    load([file_path file_name] ,'gain_prof')  
end
...
run_params.input_power_value = -130; % power at the sample

run_params.detuning_point_start = -25; % in MHz
run_params.detuning_point_end = -5; % in MHz. 
run_params.detuning_point_step = 0.5; % in MHz.
run_params.expected_detuning_number = abs((run_params.detuning_point_start - run_params.detuning_point_end)/ run_params.detuning_point_step) + 1;
...
if run_params.concatenate_runs == 1
...
if ~isfield(input_params, 'run_number')
    ...
    if run_params.redo_previously_saved_run == 1
...
if m_bias_point == 1 && m_power == 1
    ...
    input_params.additional_attenuation = 36.6; % dB. Big fridge setup as of 9/13/21 - this is the attenuation from AWG output to input of fridge
    %% Analysis params
...
if m_bias_point == 1 && m_power == 1
    ...
    %% other data arrays
    data.detunings = zeros(1, 1, expected_number);
    data.recorded_res_freq = zeros(1, 1, expected_number);
    data.peripheral.awg_output_power = zeros(1, 1, expected_number);
    data.lifetime_state_1_run_data = zeros(1, 1, expected_number, run_params.number_repetitions);
    data.lifetime_state_2_run_data = zeros(1, 1, expected_number, run_params.number_repetitions);
    data.gaussian_state_1_mean_run_data = zeros(1, 1, expected_number, run_params.number_repetitions);
    data.gaussian_state_2_mean_run_data = zeros(1, 1, expected_number, run_params.number_repetitions);
    data.sigma_gaussian_run_data = zeros(1, 1, expected_number, run_params.number_repetitions);
    data.double_gaussian_fit_error_run_data = zeros(1, 1, expected_number, run_params.number_repetitions);
    data.area_gaussian_1_run_data = zeros(1, 1,expected_number, run_params.number_repetitions); 
    data.area_gaussian_2_run_data = zeros(1, 1,expected_number, run_params.number_repetitions); 
    data.theory_hist_phases_run_data = zeros(1, 1,expected_number, run_params.number_repetitions, 360/clean_RTS_bin_width); 
    data.theory_gaussian_1_run_data = zeros(1, 1,expected_number, run_params.number_repetitions, 360/clean_RTS_bin_width); 
    data.theory_gaussian_2_run_data = zeros(1, 1,expected_number, run_params.number_repetitions, 360/clean_RTS_bin_width); 
    data.switch_finder_hist_phases_run_data = zeros(1, 1,expected_number, run_params.number_repetitions, 360/clean_RTS_bin_width); 
    data.switch_finder_hists_run_data = zeros(1, 1,expected_number, run_params.number_repetitions, 360/clean_RTS_bin_width); 
    data.poisson_lifetime_state_1_array = zeros(1, 1,expected_number,1);
    data.poisson_lifetime_state_2_array = zeros(1, 1,expected_number,1);
    data.poisson_error_lifetime_1_in_us_array = zeros(1, 1,expected_number, 2);
    data.poisson_error_lifetime_2_in_us_array = zeros(1, 1,expected_number, 2);
else
    %% arrays for VNA data and analysis single photon
    ...
    %% other data arrays 
    data.detunings = [data.lifetime_state_1_run_data; zeros(1, 1, expected_number)];
    data.recorded_res_freq = [data.recorded_res_freq; zeros(1, 1)];
    data.peripheral.awg_output_power = [data.peripherals.awg_output_power; zeros(1, 1, expected_number)];
    data.lifetime_state_1_run_data = [data.lifetime_state_1_run_data; zeros(1,1,expected_number, run_params.number_repetitions)]; 
    data.lifetime_state_2_run_data = [data.lifetime_state_2_run_data; zeros(1,1,expected_number, run_params.number_repetitions)]; 
    data.gaussian_state_1_mean_run_data = [data.gaussian_state_1_mean_run_data; zeros(1,1,expected_number, run_params.number_repetitions)]; 
    data.gaussian_state_2_mean_run_data = [data.gaussian_state_2_mean_run_data; zeros(1,1,expected_number, run_params.number_repetitions)]; 
    data.sigma_gaussian_run_data = [data.sigma_gaussian_run_data; zeros(1,1,expected_number, run_params.number_repetitions)]; 
    data.double_gaussian_fit_error_run_data = [data.double_gaussian_fit_error_run_data; zeros(1,1,expected_number, run_params.number_repetitions)]; 
    data.area_gaussian_1_run_data = [data.area_gaussian_1_run_data; zeros(1,1,expected_number, run_params.number_repetitions)]; 
    data.area_gaussian_2_run_data = [data.area_gaussian_2_run_data; zeros(1,1,expected_number, run_params.number_repetitions)]; 
    data.theory_hist_phases_run_data = [data.theory_hist_phases_run_data; zeros(1,1,expected_number, run_params.number_repetitions, 360/clean_RTS_bin_width)]; 
    data.theory_gaussian_1_run_data = [data.theory_gaussian_1_run_data; zeros(1,1,expected_number, run_params.number_repetitions, 360/clean_RTS_bin_width)]; 
    data.theory_gaussian_2_run_data = [data.theory_gaussian_2_run_data; zeros(1,1,expected_number, run_params.number_repetitions, 360/clean_RTS_bin_width)]; 
    data.switch_finder_hist_phases_run_data = [data.switch_finder_hist_phases_run_data; zeros(1,1,expected_number, run_params.number_repetitions, 360/clean_RTS_bin_width)]; 
    data.switch_finder_hists_run_data = [data.switch_finder_hists_run_data; zeros(1,1,expected_number, run_params.number_repetitions, 360/clean_RTS_bin_width)]; 
    data.poisson_lifetime_state_1_array = [data.poisson_lifetime_state_1_array; zeros(1,1,expected_number,1)];
    data.poisson_lifetime_state_2_array = [data.poisson_lifetime_state_2_array; zeros(1,1,expected_number,1)];
    data.poisson_error_lifetime_1_in_us_array = [data.poisson_error_lifetime_1_in_us_array; zeros(1,1,expected_number, 2)];
    data.poisson_error_lifetime_2_in_us_array = [data.poisson_error_lifetime_2_in_us_array; zeros(1,1,expected_number, 2)];
end
    
%% loop initialization
detuning_point = run_params.detuning_point_start;
m_detuning = 1;
res_freq = 5.9e9; % initialize well outside cCPT tunability
...
while detuning_point < run_params.detuning_point_end + run_params.detuning_point_step
    ...
    data.peripheral.bias_point_offset_and_periods(m_power, m_bias_point) = bias_point_struct;
    data.peripheral.gain_profile(m_power, m_bias_point) = gain_profile_struct;
