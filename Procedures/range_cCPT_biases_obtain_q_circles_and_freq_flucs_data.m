...
for m_flux = 1 : input_params.flux_number
    ...
    for m_ng = 1 : input_params.ng_number
        ...
        data.bias_point_set.flux_voltage_measured (m_flux, m_ng) = dmm_get_voltage(dmm_handle_1);
        data.bias_point_set.gate_voltage_measured (m_flux, m_ng) = dmm_get_voltage(dmm_handle_2);
...
%         [index_row,index_col]=max(abs(phase_diff));
        ...
        data.rough_resonance_freq (m_flux, m_ng) = temp.rough.vna_freq(temp.index_col);
        vna_set_marker_freq(vna_handle, 1,data.rough_resonance_freq (m_flux, m_ng),1);
        %% set VNA zoom data
        ...
        vna_send_average_trigger(vna);
        [temp.zoom.vna_rough_freq, temp.zoom.vna_rough_amp] = vna_get_data(vna,1,1);
        [~, temp.zoom.vna_rough_phase]=vna_get_data(vna,1,2);
...
%             disp('switch to SA')
    ...
    clear_instruments
    save('q_circle_and_freq_flucs_data.mat')
end
switch_vna_measurement