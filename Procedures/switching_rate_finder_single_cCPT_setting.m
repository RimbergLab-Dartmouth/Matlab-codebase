...
if vna_data_acquisition == 1
    ...
    [~,manual_index] = min(squeeze(data.vna.single_photon.rough.amp(m_power, m_bias_point,:)) ...
            - gain_prof.amp');
    rough_resonance = squeeze(data.vna.single_photon.rough.freq(m_power, m_bias_point, manual_index));
    ...
    plot(squeeze(data.vna.single_photon.fine.freq(m_power, m_bias_point, :)), ...
            ...
            'DisplayName', 'data')
    plot(squeeze(data.peripheral.expected_freq (m_power, m_bias_point)), min(squeeze(data.vna.single_photon.fine.amp(m_power, m_bias_point,:))), ...
    ...
    subplot(1, 3, 1)
    plot(squeeze(data.vna.single_photon.fine.real(m_power, m_bias_point, :)), ...
...
if vna_data_acquisition == 1
    ...
    plot(squeeze(data.vna.actual_power.fine.freq(m_power, m_bias_point, :)), ...
            ...
            'DisplayName', 'data')
    plot(squeeze(data.peripheral.expected_freq (m_power, m_bias_point)), min(squeeze(data.vna.actual_power.fine.amp(m_power, m_bias_point,:))), ...
        'ko', 'markersize', 4, 'linewidth', 4)
    xlabel('Freq (GHz)', 'interpreter', 'latex')
    ylabel('$\vert S_{21} \vert$(dB)', 'interpreter', 'latex')
    title('gain profile log mag single photon drive')
    ...
    subplot(1, 3, 1)
    plot(squeeze(data.vna.actual_power.fine.real(m_power, m_bias_point, :)), ...
