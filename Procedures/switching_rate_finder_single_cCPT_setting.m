...
%     rough_resonance = 5.813e9;
    ...
    plot(squeeze(data.vna.single_photon.fine.freq(m_power, m_flux, m_gate, :)), ...
            ...
            'DisplayName', 'data')
    plot(squeeze(data.peripheral.expected_freq (m_power, m_flux, m_gate)), min(squeeze(data.vna.single_photon.fine.amp(m_power, m_flux, m_gate,:))), ...
...
if vna_data_acquisition == 1
    ...
    plot(squeeze(data.vna.actual_power.fine.freq(m_power, m_flux, m_gate, :)), ...
            ...
            'DisplayName', 'data')
    plot(squeeze(data.peripheral.expected_freq (m_power, m_flux, m_gate)), min(squeeze(data.vna.actual_power.fine.amp(m_power, m_flux, m_gate,:))), ...
        'ko', 'markersize', 4, 'linewidth', 4)
    xlabel('Freq (GHz)', 'interpreter', 'latex')
    ylabel('$\vert S_{21} \vert$(dB)', 'interpreter', 'latex')
    title('gain profile log mag single photon drive')
...
data.center_freq(m_power, m_flux, m_gate, m_detuning) = res_freq + detuning_point*1e6;
input_params.sig_gen.input_LO_power(m_power, m_flux, m_gate, m_detuning) = 17; % input power to IQ4509 input mixer is 15dBm
input_params.sig_gen.input_LO_freq (m_power, m_flux, m_gate, m_detuning) = data.center_freq(m_power, m_flux, m_gate, m_detuning) - ...
    input_params.awg.input_IF_waveform_freq;
n5183b_set_frequency(keysight_sg, squeeze(input_params.sig_gen.input_LO_freq (m_power, m_flux, m_gate, m_detuning)))
n5183b_set_amplitude(keysight_sg, squeeze(input_params.sig_gen.input_LO_power(m_power, m_flux, m_gate, m_detuning)))
% the IF to IQ4509 is at 84MHz, defined in the above AWG waveforms
input_params.sig_gen.output_LO_power(m_power, m_flux, m_gate, m_detuning) = 16.8; % input power to mini circuits ZXF05 - 73L+ output mixer is 15dBm
input_params.sig_gen.output_LO_freq (m_power, m_flux, m_gate, m_detuning) = data.center_freq(m_power, m_flux, m_gate, m_detuning) + ...
    input_params.if_freq;
e8257c_set_frequency(e8257c_sig_gen, squeeze(input_params.sig_gen.output_LO_freq (m_power, m_flux, m_gate, m_detuning)))
e8257c_set_amplitude(e8257c_sig_gen, squeeze(input_params.sig_gen.output_LO_power(m_power, m_flux, m_gate, m_detuning)))
...
if run_params.save_data_and_png_param == 1 && ~run_params.analysis_during_acquisition
    ...
    raw_data_matrix.input_power_number(m_save_data_index) = m_power;
