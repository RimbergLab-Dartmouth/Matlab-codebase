...
for m_phase = 1:num_phase
    ...
    %initialize arrays
    data.lockin_x_quadrature = zeros(length(data.probe_freq), input_params.repetition_number, num_phase);
    ...
    for m_freq = 1 : length(data.probe_freq)
        ...
        for m_rep = 1 : input_params.repetition_number
            disp(['running m_phase = ' num2str(m_phase) ' of ' num2str(num_phase) ', m_freq = ' num2str(m_freq) ...
                ' of ' num2str(length(data.probe_freq)) ', m_rep = ' num2str(m_rep) ' of ' num2str(input_params.repetition_number)])
            pause(5 * input_params.lockin.time_constant);
            data.lockin_x_quadrature(m_freq, m_rep, m_phase) = sr844_lockin_query_measured_value(lockin_sr844,'X');
            data.lockin_y_quadrature(m_freq, m_rep, m_phase) = sr844_lockin_query_measured_value(lockin_sr844,'Y');
        end
    end
    
    %%%% mean of repetitions
    analysis.lockin_x_mean(m_phase,:) = mean(squeeze(data.lockin_x_quadrature(:, :, m_phase)), 2);
    analysis.lockin_y_mean(m_phase,:) = mean(squeeze(data.lockin_y_quadrature(:, :, m_phase)), 2);
    ...
    savefig(x_quad_fig_vs_y_quad, [cd '/d' input_params.file_name_time_stamp '_error_signal_acquisition/x_Vs_y_at_',num2str(current_phase),'_degree_offset.fig'])
end
